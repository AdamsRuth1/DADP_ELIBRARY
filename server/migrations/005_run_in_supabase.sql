...
-- Step 2: Enable Row Level Security (recommended)
...
-- Step 6: (Optional) Add category_id column to books table if not exists
-- Run this only if you want to link books to categories table
DO $$
BEGIN
  IF NOT EXISTS (
...
SELECT 'Books with category_id:' AS info;
SELECT COUNT(*) AS count FROM public.books WHERE category_id IS NOT NULL;
